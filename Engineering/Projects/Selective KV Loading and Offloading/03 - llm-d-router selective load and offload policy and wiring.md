---
title: llm-d-router selective load and offload policy and wiring
date: 2026-09-03
...
dependency: 02 - vLLM primary-tier selective loading
related_issue: llm-d/llm-d-router#1952
---

# llm-d-router selective load and offload policy and wiring

## Purpose

This is a technical implementation guide for the llm-d-router half of selective KV loading and offloading. The router already knows how much of a request prefix is cached by tier on each candidate endpoint, but it does not produce or reliably propagate vLLM’s per-request `kv_transfer_params.kv_load_tiers` and `max_offload_tokens` controls.

The desired behavior is:

> After endpoint selection, decide whether loading cached KV from that endpoint’s offload tiers is likely to be better than recomputation and, independently, how much newly computed KV is worth offloading. Inject both decisions into the request that actually executes on that endpoint.

The engine implementation and promotion semantics are specified in [[02 - vLLM primary-tier selective loading]].
...
### Existing offload-direction control

vLLM already accepts `kv_transfer_params.max_offload_tokens`. Omitted or `null` means uncapped offload, `0` disables offload for the request, and a positive integer caps eligibility to the first `N` request tokens subject to chunk/alignment rules. Invalid values warn and become uncapped.

No reference to `max_offload_tokens` exists in the inspected router repository. Therefore “the offload direction exists” means the vLLM execution contract exists; router policy and end-to-end wiring remain part of this contribution. The field controls amount, not destination tier.

Open vLLM PR [#52397](https://github.com/vllm-project/vllm/pull/52397) fixes a partial-tail assertion failure and a `0` handling bug. Router emission must be capability-gated until that fix or an equivalent is deployed.
...
### Independent load and offload outputs

The router must model two optional outputs, not one cache-use boolean:

| Load | Offload | Wire result |
|---|---|---|
| deny | deny | `kv_load_tiers: []`, `max_offload_tokens: 0` |
| deny | allow | `kv_load_tiers: []`, offload cap omitted or positive |
| allow | deny | load tiers omitted/allowed, `max_offload_tokens: 0` |
| allow | allow | load tiers omitted/allowed, offload cap omitted or positive |

Do not derive the offload result from the loading threshold. Current-request reuse value and future retention value are different predictions.
...
When enabled and able to decide, each router policy output is authoritative and should replace the corresponding client-provided `kv_load_tiers` or `max_offload_tokens` value. Allowing an external client to override it defeats calibration, can force expensive I/O, and may reach connector behavior outside the deployment operator’s intent.
...
The first router contribution should consume per-endpoint precise-prefix data for a binary load decision and expose an independent static offload mode:
...
- otherwise omit the load field and let existing vLLM load selection operate.
- independently configure offload as `preserve` (omit), `disable` (`max_offload_tokens: 0`), or `cap` (positive token limit).
...
The initial production policy may expose three operator-controlled modes:
...
Keep this request-scoped in the decision interface even if the first configuration is static. A later evaluator can vary it using predicted future reuse or workload class. The static load threshold must not change it.
...
6. overwrite `kv_load_tiers` when the load decision is authoritative;
7. independently overwrite `max_offload_tokens` when the offload decision is authoritative;
8. write through `MutatePayloadMap`;
9. attach separate load/offload trace fields and increment bounded counters.
...
Create one small trusted helper for inserting the validated `kv_load_tiers` and `max_offload_tokens` decisions into a router-generated transfer map. Use it from every branch that constructs or reconstructs `kv_transfer_params`, rather than duplicating merge logic.
...
- load decisions: unknown / recompute / load;
- offload decisions: preserve / disable / cap;
...
9. all four independent load/offload combinations produce the expected sibling fields;
10. `max_offload_tokens` zero and positive caps survive mutation independently of `kv_load_tiers`;
11. disabled/default configuration produces no mutation;
12. incompatible engine capability produces no mutation.
...
- Emit only `[]` or omit for loading.
- Add independent `preserve` / `disable` / `cap` offload output using vLLM’s existing `max_offload_tokens` contract.
- Add policy/director tests and separate bounded load/offload metrics.

---
title: vLLM primary-tier selective loading
date: 2026-09-03
...
companion: 03 - llm-d-router selective load and offload policy and wiring
---

# vLLM primary-tier selective loading
...
The companion router design is [[03 - llm-d-router selective load and offload policy and wiring]]. The intended responsibility split is:

- llm-d-router independently decides whether reuse is worthwhile for the selected endpoint and how much newly computed KV is eligible for offload, then sends request-scoped policy.
...
The primary-tier change in this guide is a load-path feature and must not alter storage, eviction, cascading, or retention. The broader project also wires vLLM’s existing per-request store cap, `max_offload_tokens`; no new vLLM offload API is required here. “Lazy offload” — postponing GPU-to-CPU storage until GPU pressure — remains a different contribution.
...
vLLM already implements per-request selective offload through `kv_transfer_params.max_offload_tokens`, introduced by PR #39983. The scheduler parses it into `RequestOffloadState.max_offload_tokens` and caps the store boundary in `_calc_num_offloadable_tokens`.

| Value | Store-path meaning |
|---|---|
| omitted or `null` | uncapped per-request offload |
| `0` | disable offload for the request |
| positive integer `N` | only the first `N` request tokens are eligible, subject to chunk/alignment rules |
| negative, boolean, or non-integer | warn and fall back to uncapped behavior |

This field controls how much is stored, not which configured tier receives it. It is independent of `kv_load_tiers`: a request may recompute instead of loading and still offload the recomputed prefix for future reuse, or it may load existing KV without offloading new KV.

Open PR [#52397](https://github.com/vllm-project/vllm/pull/52397) fixes a partial-tail assertion failure when the cap is below the partial-tail boundary and a path where `0` is treated as falsy/no-cap. Selective-loading work does not duplicate that PR, but integration and rollout tests must account for it.

### Required interaction matrix

| Load policy | Store policy | Required behavior |
|---|---|---|
| `kv_load_tiers=[]` | `max_offload_tokens=0` | recompute; do not store |
| `kv_load_tiers=[]` | cap omitted/positive | recompute; store according to the independent cap |
| load default/allowed | `max_offload_tokens=0` | reuse may load; do not store newly eligible KV |
| load default/allowed | cap omitted/positive | existing load and store behavior |

The primary-tier change must not derive or mutate `max_offload_tokens`.
...
Direct inspection of the local vLLM checkout, the confirmed bidirectional router requirement, and the existing `max_offload_tokens` contract at `0e3ac4907d21e77cb4781338c49fef17bfea8f2b`, PR #48123 and its review discussion, open-PR duplicate searches, and the existing CPU/tiering test suites on 2026-09-03. This is a proposed design, not a description of functionality already present.
...
- [[Factory pattern and lazy registries in vLLM KV offloading]] — the Factory family (simple/method/abstract/registry) explained from first principles, then traced through vLLM's stacked KV-offload registries with verbatim code: lazy loaders, out-of-tree escape hatches, and why `**config` splatting makes tier keys fail-closed while `extra_config.get()` keys fail silently.

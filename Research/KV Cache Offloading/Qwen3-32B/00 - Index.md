...
The selective router-to-vLLM control path works for Qwen3-32B, and the crossover sweep created substantial pressure in the external restore path. At C96, forced loading delivered 27.6% less successful request throughput than recomputation for a 512-token reusable prefix and 36.8% less for a 1,024-token prefix. The sweep does not establish a defensible static threshold because results were non-monotonic, useful external reuse remained low, NVMe was saturated, and one replica on `gjfjh` repeatedly ran 3–5× slower.

Exclude `gjfjh` with node affinity until it passes a separate balance diagnostic. The next calibration should run each focused 1K–8K prefix/concurrency/policy cell once, sequentially on the same known-balanced node, with a smaller external corpus before selecting a threshold.

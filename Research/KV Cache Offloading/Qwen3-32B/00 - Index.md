...
- [[2026-09-14 - Qwen3-32B selective-loading low-pressure validation]] — Two-run low-pressure C4 validation of the exact combined 1,024-token plus 8-waiting-request policy against current external-loading behavior. The policy preserved external KV restores, stayed below the archived queue threshold, and matched output throughput, TTFT, and ITL with zero errors. Includes seven PR-ready and mechanism-focused Vega-Lite plots.
...
A separate low-pressure C4 control confirms the safety side of the policy: with an 8K reusable prefix, combined `1024 + 8` preserved external KV transfer and matched current loading behavior (+1.34% output throughput, -3.80% mean TTFT, and -1.02% mean ITL, with zero errors). Archived waiting samples peaked at 6, below the close threshold. Together, the matrices support a deployment-specific policy that improves pressured cells without a measurable low-pressure penalty.
